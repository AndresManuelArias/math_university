...
function convertirDeterminante2X2(nuevaMatriz2x2:number[][]):string{
...
function convertir_a_matrix(nuevaMatriz2x2:number[][]):string{
    return `left lline matrix{ ${nuevaMatriz2x2.map((fila)=>fila.join("#")).join("##")}}right rline`
 }

function convertir_laplace(vector:number[][],fila:number):string{
